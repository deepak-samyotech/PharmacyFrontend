...
function SupplierBalance() {
    ...
    const fetchData = async () => {
      try {
        ...
        const transformedData = response.data?.data?.map((item) => ({
          ...
          supplierName: item.supplier_name || 0,
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(
      "<html><head><title>Supplier Balance Data</title></head><body>"
    );
    printWindow.document.write("<h1>Supplier Balance Data</h1>");
  ...
  const handleExportCSV = () => {
    ...
    link.href = csvUrl;
    link.setAttribute("download", "supplier_balance_data.csv");
                                  ...
                                  <StyledTableCell key={column.id} align={column.align}>
                                    ...
                                    {column.id === 'actions' ? (
                                      <div style={{ display: 'flex', justifyContent: 'center', width: '100px', alignItems: 'center' }}>
                                        <IconButton onClick={handleButtonClick1}>
                                          <NavigationIcon />
                                        </IconButton>
                                      </div>
                                    ) : (
