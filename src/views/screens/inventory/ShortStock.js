...
function ShortStock() {
    ...
    const fetchData = async () => {
      try {
        ...
        const transformedData = response.data?.data
          ...
          .filter((item) => item.QtyAvailable < 100); // Filter data where QtyAvailable is less than 100
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(
      "<html><head><title>Stort Stock</title></head><body>"
    );
    printWindow.document.write("<h1>Stort Stock</h1>");
  ...
  const handleExportCSV = () => {
    ...
    link.href = csvUrl;
    link.setAttribute("download", "shortStock.csv");
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Stort Stock");
    XLSX.writeFile(workbook, "shortStock.xlsx");
  };
