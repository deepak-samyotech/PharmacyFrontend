...
function SoonExpiring() {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(
      "<html><head><title>Soon Expiry Data Management</title></head><body>"
    );
    printWindow.document.write("<h1>Soon Expiry Data Management</h1>");
  ...
  const handleExportCSV = () => {
    ...
    link.href = csvUrl;
    link.setAttribute("download", "SoonExpiryData.csv");
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Soon Expiry Data");
    XLSX.writeFile(workbook, "SoonExpiryData.xlsx");
  };
  ...
  const handleExportPDF = () => {
    ...
    doc.text("Soon Expiry Data", 10, 10);
    doc.autoTable({
      columns: columns,
      body: rows,
      startY: 20,
