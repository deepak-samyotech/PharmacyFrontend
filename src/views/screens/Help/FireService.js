...
function FireService() {
  ...
  const handleExportCSV = () => {
    ...
    link.href = csvUrl;
    link.setAttribute("download", "FireServices.csv");
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Fire Services");
    XLSX.writeFile(workbook, "FireServices.xlsx");
  };
  ...
  const handleExportPDF = () => {
    ...
    const doc = new jsPDF();
    doc.text("Fire Services", 10, 10);
    doc.autoTable({
      head: [columns],
      body: rows,
    });
    doc.save("FireServices.pdf");
  };

  const handlePrint = () => {
    const printWindow = window.open("", "_blank");
    printWindow.document.write(
      "<html><head><title>Fire Services</title></head><body>"
    );
    printWindow.document.write("<h1>Fire Services</h1>");
