...
function Hospital() {
  ...
  const handleExportCSV = () => {
    ...
    link.href = csvUrl;
    link.setAttribute("download", "HospitalData.csv");
  ...
  const handleExportExcel = () => {
    ...
    XLSX.writeFile(workbook, "HospitalData.xlsx");
  };
  ...
  const handleExportPDF = () => {
    ...
    const doc = new jsPDF();
    doc.text("Hospital Data", 10, 10);
    doc.autoTable({
      head: [columns],
      body: rows,
    });
    doc.save("HospitalData.pdf");
  };

  const handlePrint = () => {
    const printWindow = window.open("", "_blank");
    printWindow.document.write(
      "<html><head><title>Hospital Data</title></head><body>"
    );
    printWindow.document.write("<h1>Hospital Data</h1>");
