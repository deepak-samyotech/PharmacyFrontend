...
function Police() {
  ...
  const handleExportCSV = () => {
    ...
    link.href = csvUrl;
    link.setAttribute("download", "PoliceData.csv");
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Police Data");
    XLSX.writeFile(workbook, "PoliceData.xlsx");
  };
  ...
  const handleExportPDF = () => {
    ...
    const doc = new jsPDF();
    doc.text("Police Data", 10, 10);
    doc.autoTable({
      head: [columns],
      body: rows,
    });
    doc.save("PoliceData.pdf");
  };

  const handlePrint = () => {
    const printWindow = window.open("", "_blank");
    printWindow.document.write(
      "<html><head><title>Police Data</title></head><body>"
    );
    printWindow.document.write("<h1>Police Data</h1>");
