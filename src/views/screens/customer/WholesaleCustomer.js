...
function WholesaleCustomer() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const wholesaleCustomers = transformedData.filter(
  ...
  const handleExportCSV = () => {
    ...
    link.href = csvUrl;
    link.setAttribute("download", "WholesaleCustomerDetails.csv");
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Wholesale Customer Details");
    XLSX.writeFile(workbook, "WholesaleCustomerDetails.xlsx");
  };
  ...
  const handleExportPDF = () => {
    ...
    const doc = new jsPDF();
    doc.text("Wholesale Customer Details", 10, 10);
    doc.autoTable({
      head: [columns],
      body: rows,
    });
    doc.save("WholesaleCustomerDetails.pdf");
  };

  const handlePrint = () => {
    const printWindow = window.open("", "_blank");
    printWindow.document.write(
      "<html><head><title>Wholesale Customer Details</title></head><body>"
    );
    printWindow.document.write("<h1>Wholesale Customer Details</h1>");
